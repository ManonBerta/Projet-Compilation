%{
	...
	symboleVariable *tableSymbole[TAILLE];
%}
...
expression 	: logical_or_expression {$$ = $1;}
        	| unary_expression '=' expression {$$ = set($1, $3);}
        	;

declaration	: declaration_specifiers declarator ';' {$$ = $1 inserer($2) ;}
        	| struct_specifier ';'{$$ = $1 ;}
        	;
...
typedef struct _symboleVariable{
    char *nom;
    type_t type;
    int taille;
    int position;
    double valeur;
    struct _symboleVariable *suivant;
} symboleVariable;

symboleVariable * ajouter (char* nom);
symboleVariable * rechercher (char * nom);
...
double rechercher (char *nom)
{
for (int i=0; i<taille; i++){
	if(nom == tableSymbole[i].nom)
	{ return tableSymbole[i].valeur;}}
yyerror("valeur non trouver");
}
void set (char *nom, double cara)
{
for (int i=0; i<taille; i++)
{if(nom == tableSymbole[i].nom)
	{tableSymbole[i].valeur = cara;
	return;}}
yyerror("valeur non trouve");
}
...
symbole * inserer( char *nom ) {
  ...
  if ( precedent == NULL ) {
    tableSymbole[h] = (symboleVariable *) malloc(sizeof(symboleVariable));
    s = tableSymbole[h];
  }}
